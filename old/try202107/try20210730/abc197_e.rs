...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut v = vec![BinaryHeap::new(); n + 1];
    for (x, c) in xc {
        v[c as usize].push(x);
    }
    ...
    for i in 1..=n {
        ...
        if v[i].len() == 1 {
            let k = v[i].pop().unwrap();
            minmax.push((k, k));
            continue;
        }
        let max = v[i].pop().unwrap();
        let mut min = v[i].pop().unwrap();
        while let Some(k) = v[i].pop() {
            min = k;
        }
        minmax.push((min, max));
    }
    ...
    // 0..len -> min len..len*2 -> max
    let l = minmax.len();
    let mut g = vec![Vec::new(); l * 2];
    g[0].push((l, 0));
    g[l].push((0, 0));
    for i in 1..l {
        let (mib, mab) = minmax[i - 1];
        let (mia, maa) = minmax[i];
        g[i - 1].push((i, (mib - maa).abs() + (maa - mia).abs()));
        g[i + l - 1].push((i + l, (mab - mia).abs() + (maa - mia).abs()));
        g[i - 1].push((i + l, (mib - mia).abs() + (maa - mia).abs()));
        g[i + l - 1].push((i, (mab - maa).abs() + (maa - mia).abs()));
    }
    g[l - 1].push((l * 2 - 1, 0));
    g[l * 2 - 1].push((l - 1, 0));
    ...
    q.push_back((0, 0));
    const INF: i64 = 1 << 62;
    let mut dist = vec![INF; l * 2];
    dist[0] = 0;
    while let Some((from, c)) = q.pop_front() {
        if dist[from] != c {
            continue;
        }
