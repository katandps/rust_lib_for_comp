...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    println!("{}", s(n, &g, &d, &mut memo));
}

const M: usize = 1_000_000_007;

fn s(to: usize, g: &Vec<Vec<usize>>, d: &Vec<usize>, memo: &mut HashMap<usize, usize>) -> usize {
    if let Some(r) = memo.get(&to) {
        return *r;
    }
    if to == 1 {
        return 1;
