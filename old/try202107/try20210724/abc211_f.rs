...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for i in 0..n {
        ...
        v.sort();
        for i in 0..v.len() {
            edges.push((v[i].0, v[i].1, v[i].2, v[i].3 * v[0].3));
        }
    }
    edges.sort();
    ...
    let mut ans = vec![0; q];
    let mut bit = BinaryIndexedTree::new(100020);

    // dbg!(&edges);
    // dbg!(&bit);
    for i in 0..q {
        let (xq, yq, i) = queries[i];
        while let Some((x, y1, y2, a)) = edges.pop_front() {
            if x > xq {
                edges.push_front((x, y1, y2, a));
                break;
            }
            // dbg!(x, y1, y2, a);
