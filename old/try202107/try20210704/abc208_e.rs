...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    println!("{}", sol(n, k, false, &mut memo) - 1);
    //
    // let mut a = 0;
    // for i in 1..=n {
    //     let s = i
...
fn sol(n: usize, k: usize, non_0: bool, memo: &mut HashMap<(usize, usize, bool), usize>) -> usize {
    if let Some(&u) = memo.get(&(n, k, non_0)) {
        return u;
    }
    if n < 10 {
        let ret = min(n + 1, k + 1);
        memo.insert((n, k, non_0), ret);
        ret
    } else {
        let s = n
            .to_string()
        ...
        if non_0 {
            ...
            ret += sol(p - 1, k, non_0, memo);
        }
        // 0 < i < s[0]
        for i in 1..s[0] {
            ret += sol(p - 1, k / i, true, memo);
        }
        // i = s[0]
        if s[1] == 0 {
            ret += n - p * s[0] + 1;
        } else {
            ret += sol(n - p * s[0], k / s[0], true, memo);
        }
        memo.insert((n, k, non_0), ret);
        ret
    }
