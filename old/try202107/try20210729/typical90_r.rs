...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let t = reader.f();
    let (l, x, y) = (reader.f(), reader.f(), reader.f());
    let q = reader.u();
    let e = reader.uv(q);

    for i in 0..q {
        let theta = e[i] as f64 / t * 2.0 * std::f64::consts::PI;
        let (yi, zi) = (-l / 2.0 * theta.sin(), l / 2.0 - l / 2.0 * theta.cos());
        let dist = (x * x + (y - yi) * (y - yi)).sqrt();
        let height = zi;
        println!("{}", height.atan2(dist) / std::f64::consts::PI * 180.);
    }
}
