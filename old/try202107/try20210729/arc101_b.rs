...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut ok = 0;
    let mut ng = 1_000_000_001i64;
    while (ok - ng).abs() > 1 {
        let mid = (ok + ng) / 2;

        let mut bit = BinaryIndexedTree::new(200010);
        const OFFSET: i64 = 100005; // 累積和が負になることがある
        let mut pattern = 0; // 中央値がmid以上になる区間の数
        let mut to = OFFSET; // i番目まで見たときの累積和
        bit.add(to as usize, 1); // 一つも数列の中身を見ていないときの累積和は0
        for i in 0..n {
            to += if a[i] >= mid { 1 } else { -1 };
            pattern += bit.sum(to as usize);
            bit.add(to as usize, 1);
        }
        if pattern >= ((n * (n + 1) / 2 + 1) as i64 / 2) {
            ok = mid;
        } else {
            ng = mid;
        }
