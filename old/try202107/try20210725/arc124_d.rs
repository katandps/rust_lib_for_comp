...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut ans = 0;
    let mut n_only = Vec::new();
    let mut m_only = Vec::new();
    for l in &loops {
        let mut n_zone = false;
        let mut m_zone = false;
        for &li in l {
            if li <= n {
                n_zone = true;
            } else {
    ...
    for i in 0..max(n_only.len(), m_only.len()) {
        match (n_only.get(i), m_only.get(i)) {
            (Some(n), Some(m)) => ans += n + m,
            (Some(n), None) => ans += n + 1,
            (None, Some(m)) => ans += m + 1,
            (None, None) => {}
        }
