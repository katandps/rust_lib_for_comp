...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    // let (h, w, c) = (500, 500, 500);
    // let a = vec![vec![1; w]; h];

    const INF: i64 = std::i64::MAX;
    let c = c as i64;
    let mut ans = INF;
    let mut mines = vec![INF; w];
    for x1 in 0..w {
        for x2 in x1 + 1..w {
            ans = min(ans, (x2 as i64 - x1 as i64) * c + a[0][x1] + a[0][x2]);
        }
        mines[x1] = a[0][x1] + c;
    }
    ...
    for y in 1..h {
        for x1 in 0..w {
            for x2 in 0..w {
                if x2 > x1 {
                    ans = min(ans, (x2 as i64 - x1 as i64) * c + a[y][x1] + a[y][x2]);
                }
                ans = min(
                    ans,
                    (x1 as i64 - x2 as i64).abs() * c + mines[x1] + a[y][x2],
                );
            }
            mines[x1] = min(mines[x1] + c, a[y][x1] + c);
        }
    }
    println!("{}", ans);
}
