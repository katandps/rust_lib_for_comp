...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut ok = 1;
    let mut ng = n as i64 * n as i64 * 1000000 + 1;
    let mut ok_c = 0;
    while (ok - ng).abs() > 1 {
        let mid = (ok + ng) / 2;
        let mut c = 0;
        for i in 0..n {
            let mut ok_i = -1;
            let mut ng_i = n as i64;
            while (ok_i - ng_i).abs() > 1 {
                let mid_i = (ok_i + ng_i) / 2;
                if a[mid_i as usize] + a[i] >= mid {
                    ok_i = mid_i
                } else {
                    ng_i = mid_i
                }
    ...
    for i in 0..n {
        let mut oki = -1;
        let mut ngi = n as i64;
        while (oki - ngi).abs() > 1 {
            let midi = (oki + ngi) / 2;
            if a[midi as usize] + a[i] > ok {
                oki = midi
            } else {
                ngi = midi
            }
        }
        ans += a[i] * ngi + s[ngi as usize];
    }
    ans += (m as i64 - ok_c) * ok;
    println!("{}", ans);
}
