...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let m = s + 1;
    // dp[index][red_sum] = patterns;
    let mut dp = vec![vec![Mi::new(0); m]; n + 1];
    // dp[index][red_sum] = patterns; (green = 0)
    let mut dp2 = vec![vec![Mi::new(0); m]; n + 1];
    dp[0][0] += 1;
    dp2[0][0] += 1;
    for i in 0..n {
        let ai = a[i];
    ...
    let mut ans = Mi::new(3).pow(n as i64);
    for i in (s + 1) / 2..=s {
        ans -= dp[n][i] * 3;
    }
    if s % 2 == 0 {
