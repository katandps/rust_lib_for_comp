...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let (n, a, b, c) = reader.i4();
    let mvt = ModValTable::new(n as usize * 2);

    let mut ans = Mi::new(0);

    let pa = Mi::new(a) / (a + b);
    let pb = Mi::new(b) / (a + b);
    let pc = Mi::new(100 - c) / 100;
    for game in n..n * 2 {
        let lose = game - n;
        let combination = mvt.combination(game - 1, lose);
        ans += pa.pow(n) * pb.pow(lose) * combination * game / pc;
        ans += pb.pow(n) * pa.pow(lose) * combination * game / pc;
    }
    println!("{}", ans);
}
