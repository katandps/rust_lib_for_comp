...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let res = s(vec![0; 9], &mut map);
    println!("{}", res.0);
    println!("{}", res.1);
}

fn s(v: Vec<i32>, map: &mut HashMap<Vec<i32>, (i64, i64)>) -> (i64, i64) {
    if let Some(r) = map.get(&v) {
        return *r;
    }
    let next_is_takahashi = v.iter().filter(|&&x| x != 0).count() % 2 == 0;
    if next_is_takahashi {
        (0..9)
            .filter(|&i| v[i] == 0)
            .map(|i| {
                let mut v = v.clone();
