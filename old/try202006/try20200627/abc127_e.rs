...
fn main() {
    ...
    let combination = Combination::new(n * m - 2);
    let mut tate = ModInt::new(0);
    for i in 1..m {
        tate = tate + ModInt::new(n) * n * (m - i) * i;
    }
    let mut yoko = ModInt::new(0);
    for i in 1..n {
        yoko = yoko + ModInt::new(m) * m * (n - i) * i;
    }
    println!("{}", (tate + yoko) * combination.get(k - 2));
    //dbg!(tate, yoko, combination.get(k - 2));
}
