...
fn main() {
    ...
    let (n, a, b) = reader.u3();
    let mut h = reader.uv(n);
    h.sort();

    let is_ok = |k| -> bool {
        let mut rest_count = k;
        // dbg!(k);
        for i in 0..n {
            if h[i] < k * b {
                continue;
            ...
            let rest = h[i] - k * b;
            if rest_count * (a - b) < rest {
                return false;
            }
            rest_count -= rest / (a - b) + if rest % (a - b) > 0 { 1 } else { 0 };
            // dbg!(&rest_count, i, rest);
        }
        true
    };
    let mut ng = 0;
    let mut ok = 1_000_000_000_000_000usize;
    while (ok - ng) > 1 {
        let mid = (ok + ng) / 2;
        //dbg!(ok, ng, mid);
        if is_ok(mid) {
