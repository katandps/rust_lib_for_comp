...
pub fn solve<R: BufRead>(mut reader: StdinReader<R>) {
    ...
    for &(h, w) in &hw {
        *hcount.entry(h).or_insert(0) += 1;
        *wcount.entry(w).or_insert(0) += 1;
    }

    let mut hc = 0;
    for (_, &count) in &hcount {
        hc = max(hc, count);
    }
    let mut hm = HashSet::new();
    for (&h, &count) in &hcount {
        if count == hc {
            hm.insert(h);
        }
    }
    let mut wc = 0;
    for (_, &count) in &wcount {
        wc = max(wc, count);
    }
    let mut wm = HashSet::new();
    ...
    let mut v = Vec::new();
    for (h, w) in hw {
        if hm.contains(&h) && wm.contains(&w) {
            v.push((h, w));
            c += 1;
        }
    }
