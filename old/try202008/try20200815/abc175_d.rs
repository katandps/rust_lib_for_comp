...
pub fn solve<R: BufRead>(mut reader: StdinReader<R>) {
    ...
    //(start,end)
    let mut l = vec![(n as i64, n as i64); n + 1];
    ...
    for i in 1..=n {
        ...
        for j in 1..=n {
            let next = p[cur - 1];
            m[i][j] = m[i][j - 1] + c[next - 1];

            if memo.contains_key(&next) {
                l[i] = (*memo.get(&next).unwrap(), j as i64);
                break;
            }
            memo.insert(next, j as i64);
            cur = next;
        }
    }
    ...
    for i in 1..=n {
        ...
        if m[i][l[i].1 as usize] > 0 {
            let sum = m[i][l[i].1 as usize] * (k / l[i].1);
            let k = k % l[i].1;
            high = max(high, sum);
            for j in 1..=k as usize {
                high = max(high, sum + m[i][j]);
            }

            if sum != 0 {
                let sum = sum - m[i][l[i].1 as usize];
                high = max(high, sum);
                for j in 1..=l[i].1 as usize {
                    high = max(high, sum + m[i][j]);
                }
            }
        } else {
