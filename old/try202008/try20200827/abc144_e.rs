...
pub fn solve<R: BufRead>(mut reader: StdinReader<R>) {
    ...
    let mut f = reader.uv(n);
    a.sort();
    f.sort();
    f.reverse();

    let mut ng = -1isize;
    let mut ok = 1_000_000_000_000_000_001isize;
    while (ok - ng).abs() > 1 {
        let mid = (ok + ng) / 2;
        if is_ok(&a, &f, k, mid) {
            ok = mid
...
fn is_ok(a: &Vec<usize>, f: &Vec<usize>, k: usize, mid: isize) -> bool {
    let mut over = 0;
    for i in 0..a.len() {
        if a[i] * f[i] > mid as usize {
            over += a[i] - mid as usize / f[i];
        }
    }
    over <= k
}
