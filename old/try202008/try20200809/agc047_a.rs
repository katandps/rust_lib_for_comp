...
pub fn solve<R: BufRead>(mut reader: StdinReader<R>) {
    ...
    for _ in 0..n {
        ...
        for i in (0..s.len()).rev() {
            ...
            k += (s[i] as u8 - '0' as u8) as usize * c;
            c *= 10;
            if !b {
                p += 1;
            }
        ...
        v.push((two + 10 - p, five + 10 - p));
    }

    let mut l = vec![vec![0; 42]; 42];
    for &(t, f) in &v {
        l[t][f] += 1;
    }
    let mut s = vec![vec![0; 42]; 42];
    for i in (0..41).rev() {
        s[i][41] += l[i + 1][41];
        s[41][i] += l[41][i + 1];
    }
    for i in (0..41).rev() {
        for j in (0..41).rev() {
            s[i][j] = s[i + 1][j] + s[i][j + 1] + l[i][j] - s[i + 1][j + 1];
        }
    }
    let mut ans = 0usize;
    for i in 0..n {
        let (t, f) = v[i];
        let mut a = s[20 - min(t, 20)][20 - min(f, 20)];
        if t >= 10 && f >= 10 {
            a -= 1;
        }
        ans += a;
