...
fn main() {
    ...
    let mut last_appeard = vec![0; n + 1];
    ...
    for (l, r) in sorted {
        while rr < r {
            if last_appeard[c[rr]] > 0 {
                bit.add(last_appeard[c[rr]], -1);
            }
            bit.add(rr + 1, 1);
            last_appeard[c[rr]] = rr + 1;
            rr += 1;
        }
        let a = bit.sum(r) - bit.sum(l - 1);
        ans.insert((l, r), format!("{}", a));
    }
    for (l, r) in lr {
        println!("{}", ans.get(&(l, r)).unwrap());
    }
}
