...
fn main() {
    ...
    let total = x.iter().filter(|&&x| x == '1').count();

    let mut minor = vec![0; n + 1];
    let mut major = vec![0; n + 1];
    minor[1] = if total <= 2 { 0 } else { 1 };
    major[1] = 1;
    for i in 2..=n {
        if total != 1 {
            minor[i] = (minor[i - 1] * 2) % (total - 1);
        }
        major[i] = (major[i - 1] * 2) % (total + 1);
    }

    let mut minor_default = 0;
    let mut major_default = 0;
    for i in 0..n {
        if x[i] == '0' {
            continue;
        }
        minor_default += minor[n - i];
        major_default += major[n - i];
    }
    if total != 1 {
        minor_default %= total - 1;
    }
    major_default %= total + 1;
    ...
    for i in 0..n {
        ...
        if x[i] == '1' {
            ...
            // dbg!(minor_default + total - 1 - minor[n - i]);
            println!(
                "{}",
                count[(minor_default + total - 1 - minor[n - i]) % (total - 1)] + 1
            );
        } else {
            //major
            // dbg!(major_default + major[n - i]);
            println!(
                "{}",
                count[(major_default + major[n - i]) % (total + 1)] + 1
            )
        }
    }

    // dbg!(count, major, minor, total, major_default, minor_default);
}
