...
fn main() {
    ...
    let mut m = Vec::new();
    for _ in 0..n {
        mm.push(parse(reader.s()));
        m.push(parse(reader.s()));
    }
    ...
    for i in 0..n {
        ...
        if mm[i] >= 300 && mm[i] < 350 {
            ans[1] += 1;
        }
        if mm[i] >= 250 && mm[i] < 300 {
            ans[2] += 1;
        }
...
fn parse(mut s: Vec<char>) -> i32 {
    ...
    if s.contains(&'.') {
        ...
        for i in (0..s.len()).rev() {
            if s[i] != '.' {
                p += (s[i] as i32 - '0' as i32) * b * if minus { -1 } else { 1 };
                b *= 10;
            }
        }
        p
        ...
        let p: i32 = str.parse().unwrap();
        p * 10 * if minus { -1 } else { 1 }
    }
}
