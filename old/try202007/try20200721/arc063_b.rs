...
fn main() {
    ...
    let (n, _) = reader.u2();
    let a = reader.uv(n);
    let mut min_a = vec![std::usize::MAX; n];
    min_a[0] = a[0];
    for i in 1..n {
    ...
    for i in 1..n {
        ...
        diff.entry(Reverse(a[i] - min_a[i - 1]))
            .or_insert(HashSet::new())
            .insert((min_a[i - 1], a[i]));
    }

    let (_, entry) = diff.iter().next().unwrap();

    let &max_diff = entry.iter().next().unwrap();
    let max_diff = max_diff.1 - max_diff.0;

    let mut v = HashMap::new();
    for i in 0..n {
        v.entry(a[i]).or_insert(Vec::new()).push(a[i]);
        if a[i] > max_diff {
            v.entry(a[i] - max_diff).or_insert(Vec::new()).push(a[i]);
        }
    }
