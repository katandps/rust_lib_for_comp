...
fn main() {
    ...
    let mut train_uf = UnionFind::new(n + 1);
    let mut load_uf = UnionFind::new(n + 1);
    ...
    for i in 1..=n {
        *count
            .entry((load_uf.root(i), train_uf.root(i)))
            .or_insert(0) += 1;
    }
    for i in 1..=n {
        println!(
            "{}",
            count.get(&(load_uf.root(i), train_uf.root(i))).unwrap()
        );
    }
}
