...
fn main() {
    ...
    let (a, b) = reader.u2();
    let p = reader.uv(n);
    let s: Vec<_> = p.iter().cloned().filter(|&x| x <= a).collect();
    let m: Vec<_> = p.iter().cloned().filter(|&x| x > a && x <= b).collect();
    let l: Vec<_> = p.iter().cloned().filter(|&x| x > b).collect();
    println!("{}", min(s.len(), min(m.len(), l.len())));
}
