...
fn main() {
    ...
    let mut tmin = vec![0; n + 2];
    let mut tmax = vec![0; n + 2];
    ...
    let mut amin = vec![0; n + 2];
    let mut amax = vec![0; n + 2];
    for i in (0..n).rev() {
        if amax[i + 2] < a[i] {
            amin[i + 1] = a[i];
            amax[i + 1] = a[i];
    ...
    let mut ans = 1;
    for i in 1..=n {
        let g = min(tmax[i], amax[i]);
        let s = max(tmin[i], amin[i]);
        // dbg!(g, s);
        if g >= s {
            ans = ans * (g - s + 1) % 1_000_000_007;
        } else {
            ans *= 0;
        }
    }
    println!("{}", ans);
    // dbg!(tmin, tmax, amin, amax);
}
