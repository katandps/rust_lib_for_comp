...
fn main() {
    ...
    let mut plus: Vec<_> = a
        .iter()
        .cloned()
        .filter(|&x| x > 0)
        .map(|x| x as usize)
        .collect();
    let mut minus: Vec<_> = a
        .iter()
        .cloned()
        .filter(|&x| x < 0)
        .map(|x| (-x) as usize)
    ...
    let mut count = 0;
    while count < k {
        //マイナスが残り2つ以上あり、2つ分の絶対値の積がプラス2つより大きい場合にマイナスから取る
        if m_i + 1 < minus.len()
            && k >= 2 + count
            && (p_i + 1 >= plus.len() || (minus[m_i] * minus[m_i + 1] > plus[p_i] * plus[p_i + 1]))
        {
            ans = ans * minus[m_i] % m;
            ans = ans * minus[m_i + 1] % m;
            m_i += 2;
            count += 2;
            continue;
        }
        if zero > 0 && p_i >= plus.len() {
            println!("{}", 0);
            return;
        }
        if zero == 0 && p_i >= plus.len() {
