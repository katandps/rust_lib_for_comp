...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for _ in 0..m {
        let (a, b, c, d) = (reader.u(), reader.u(), reader.i(), reader.i());
        g[a].push((b, c, d));
        g[b].push((a, c, d));
    }
    ...
    while !heap.is_empty() {
        ...
        for &(to, c, d) in &g[from] {
            let mut ng = dist[from];
            let mut ok = INF;
            while (ng - ok).abs() > 1 {
                let mid = (ok + ng) / 2;
                if d > mid.saturating_mul(mid + 1) {
                    ng = mid;
                } else {
                    ok = mid;
                }
            }
            let total_cost = ng + d / (ng + 1) + c;
            if dist[to] > total_cost {
                dist[to] = total_cost;
                heap.push((Reverse(total_cost), to));
            }
        }
    }
    if dist[n] == INF {
