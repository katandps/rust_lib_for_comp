...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut m = vec![vec![Mi::new(0); 1 << h]; 1 << h];

    for start in 0..1 << h {
        search(start, 0, 0, h as usize, &mut m[start]);
    }

    let matrix: Matrix = m.try_into().unwrap();

    let mut initial = vec![vec![Mi::new(0); 1 << h]];
    initial[0][0] = Mi::new(1);
    let initial_m: Matrix = initial.try_into().unwrap();
    // dbg!(&matrix, &initial_m);

    let pow = matrix.pow(w).unwrap();
    // dbg!(&pow);
    let w_1 = (initial_m * pow).unwrap();

    // dbg!(&w_1);
    println!("{}", w_1.buf[0][0]);
}

fn search(from: usize, to: usize, next: usize, limit: usize, res: &mut Vec<Mi>) {
    if (1 << next) & from != 0 {
        search(from, to, next + 1, limit, res);
        return;
    }
    ...
    search(from, to, next + 1, limit, res);
    search(from, to + (1 << next), next + 1, limit, res);
    if next + 1 < limit && (1 << (next + 1) & from == 0) {
        search(from, to, next + 2, limit, res);
    }
}
