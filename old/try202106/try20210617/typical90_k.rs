...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    const LIMIT: usize = 5010;

    let n = reader.u();
    let mut dcs = reader.uv3(n);
    dcs.sort();
    let mut dcs = dcs.into_iter().collect::<VecDeque<_>>();
    let mut dp = vec![vec![0usize; LIMIT + 1]; LIMIT + 1];
    // dbg!(&dcs);
    for i in 0..LIMIT {
        dp[i + 1] = dp[i].clone();
        while let Some(&(d, c, s)) = dcs.iter().next() {
            if i + 1 < d {
                break;
            }
            dcs.pop_front();
            if c <= d {
                for j in (0..LIMIT).rev() {
                    if j + c <= d {
                        dp[i + 1][j + c] = max(dp[i + 1][j] + s, dp[i + 1][j + c]);
                    }
                }
            }
        }
