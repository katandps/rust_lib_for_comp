...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    a.sort();
    for ki in k {
        let mut ok = 1_100_000_000_000_000_000i64;
        let mut ng = 0;
        while (ok - ng).abs() > 1 {
            let mid = (ok + ng) / 2;
            let mut mini = 0;
            let mut maxi = a.len() as i64;
            while (mini - maxi).abs() > 1 {
                let midi = (mini + maxi) / 2;
                if a[midi as usize] < mid {
                    mini = midi;
                } else {
                    maxi = midi;
                }
            }
            // dbg!(ok, ng, mid, mini, maxi);
            if mid > ki + mini {
                ok = mid;
            } else {
                ng = mid;
