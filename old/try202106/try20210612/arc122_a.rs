...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut dp = vec![vec![(Mi::new(0), Mi::new(0)); n + 1]; 2];
    dp[0][0] = (Mi::new(a[0]), Mi::new(1));

    for i in 1..n {
        let pl_num = dp[0][i - 1].1 + dp[1][i - 1].1;
        let mi_num = dp[0][i - 1].1;
        dp[0][i] = (dp[0][i - 1].0 + dp[1][i - 1].0 + pl_num * a[i], pl_num);
        dp[1][i] = (dp[0][i - 1].0 - mi_num * a[i], mi_num);
    }
    println!("{}", dp[0][n - 1].0 + dp[1][n - 1].0);
    // dbg!(dp);
}
