...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mvt = ModValTable::new((n + k) as usize);
    ...
    for i in 1..=k {
        ...
        let bl = k - i;
        let red = n + 1 - k - i;
        ans *= mvt.factorial(bl + i - 1) / mvt.factorial(bl) / mvt.factorial(i - 1);
        ans *= mvt.factorial(red + i) / mvt.factorial(red) / mvt.factorial(i);
        println!("{}", ans);
    }
}
