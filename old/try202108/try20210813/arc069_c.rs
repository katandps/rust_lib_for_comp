...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    a.iter()
        ...
        .sorted()
        .for_each(|(ai, i)| {
            if !m.contains_key(&Reverse(ai)) {
                m.insert(Reverse(ai), i);
            }
            *c.entry(ai).or_insert(0) += 1;
        });

    let mut cnt = 0;
    let mut cur = None;
    let mut ans = vec![0; n];
    for (Reverse(ai), index) in m {
        if let Some((ai_b, index_b)) = cur {
            ans[index_b] += cnt * (ai_b - ai);
            cur = Some((ai, min(index, index_b)));
        } else {
            cur = Some((ai, index));
        }
        cnt += c.get(&ai).unwrap();
    }
    if let Some((ai, index)) = cur {
        ans[index] += cnt * ai;
    }
    for i in 0..n {
        println!("{}", ans[i]);
