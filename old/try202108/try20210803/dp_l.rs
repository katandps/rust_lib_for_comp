...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    println!("{}", f(0, n, &a, &mut dp));
}

// [l, r)
fn f(l: usize, r: usize, a: &Vec<i64>, dp: &mut Vec<Vec<Option<i64>>>) -> i64 {
    if let Some(ret) = dp[l][r] {
        ret
    } else {
        dp[l][r] = Some(if l + 1 == r {
            a[l]
        } else {
            max(a[l] - f(l + 1, r, a, dp), a[r - 1] - f(l, r - 1, a, dp))
        });
        dp[l][r].unwrap()
    }
}
