...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let (h, w, a, _b) = reader.u4();
    let mut dp = HashMap::new();
    println!("{}", f(0, a, 0, h, w, &mut dp));
}

// nokori(a,b) siita: state
fn f(
    state: usize,
    a: usize,
    last: usize,
    ...
    if let Some(&r) = dp.get(&(state, a)) {
        ...
        for x in 0..w {
            for y in 1..h {
                ...
                let k1 = 1 << (x + (y - 1) * w);
                let k2 = 1 << (x + y * w);
                if state & k1 > 0 || state & k2 > 0 {
                    continue;
                }
                r += f(state | k1 | k2, a - 1, k, h, w, dp);
            }
        }
        for x in 1..w {
            for y in 0..h {
