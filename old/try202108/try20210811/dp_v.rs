...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    dp1[1] = 1;
    dfs1(1, 0, m, &g, &mut dp1);
    dfs2(1, 0, m, &g, &mut dp2, &mut dp1);
    for i in 1..=n {
        println!("{}", dp2[i]);
    }
}

fn dfs1(cur: usize, parent: usize, m: usize, g: &Vec<Vec<usize>>, dp: &mut Vec<usize>) {
    dp[cur] = 1;
    g[cur].iter().for_each(|&to| {
        if to != parent {
            dfs1(to, cur, m, g, dp);
            dp[cur] *= dp[to] + 1;
            dp[cur] %= m;
