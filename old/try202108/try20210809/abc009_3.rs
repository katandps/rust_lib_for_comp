...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let sa = s.iter().map(|&c| (c as u8 - b'a') as usize).collect_vec();

    let mut p = vec![0i64; 26];
    ...
    for i in 0..n {
        ...
        for j in 0..26 {
            ...
            let d = (0..26).map(|i| max(0, p_top[i] - p[i])).sum::<i64>();
            if diff + 1 + d <= k {
                diff += 1;
                ans.push(j);
                break;
    ...
    println!("{}", ans.iter().map(|&u| (u as u8 + b'a') as char).join(""));
}
