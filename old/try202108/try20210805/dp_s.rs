...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let k = reader.digits();
    let d = reader.u();
    let mut dp = vec![vec![Mi::new(0), Mi::new(0)]; d];
    dp[0][0] += 1;
    for i in 0..k.len() {
        let ki = k[i] as usize;
        let mut new_dp = vec![vec![Mi::new(0), Mi::new(0)]; d];
        for di in 0..d {
            new_dp[(di + ki) % d][0] += dp[di % d][0];
        }
        for di in 0..d {
            for k in 0..10 {
                if k < ki {
                    new_dp[(di + k) % d][1] += dp[di % d][0];
                }
                new_dp[(di + k) % d][1] += dp[di % d][1];
            }
        }
        dp = new_dp;
    }
    println!("{}", dp[0][0] + dp[0][1] - 1);
}
