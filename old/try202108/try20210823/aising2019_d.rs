...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for _ in 0..q {
        let x = reader.i();
        let mut t_get = n as i64;
        let mut t_cant = n as i64 / 2 - 1;
        while (t_get - t_cant).abs() > 1 {
            let mid = ((t_get + t_cant + 1) / 2) as usize;
            let t_min = a[mid];
            let a_min = a[1 + mid * 2 - n];
            if (x - t_min).abs() >= (x - a_min).abs() {
                t_get = mid as i64;
            } else {
                t_cant = mid as i64;
            }
        }
        let o = (2 * t_get as usize).saturating_sub(n);
        let ans = s[n] - s[t_get as usize] + if n % 2 == 1 { odd[o] } else { s[o] - odd[o] };
        println!("{}", ans);
    }
}
