...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let a: usize = reader.n();
    let b: usize = reader.n();
    let x: usize = reader.n();

    if x >= a * 1_000_000_000 + b * 10 {
    ...
    for i in 1..10 {
        ...
        let rest = (x - b * i) / a;
        // dbg!(rest, 10usize.pow(i as u32 - 1), 10usize.pow(i as u32) - 1);
        if rest < 10usize.pow(i as u32 - 1) {
            continue;
        }
        ans = max(ans, min(rest, (10usize.pow(i as u32) - 1) as usize));
    }
    println!("{}", ans);
}
