...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut xyz = Vec::new();
    for _ in 0..n {
        xyz.push((reader.i(), reader.i(), reader.i()));
    }
    ...
    //dp[c][i] = cost
    //cは現在地 iのjビット目が立っているものを、jを訪れたものとみなす
    let mut dp = vec![vec![1_000_000_000_000; 1 << n]; n];
    dp[0][0] = 0;
    ...
    q.push_back((0, 0));
    while !q.is_empty() {
        let (from, state) = q.pop_front().unwrap();
        if state == (1 << n) - 1 {
            ans = min(ans, dp[from][state] + cost[from][0])
        }
        ...
        for to in 0..n {
            ...
            let next_cost = dp[from][state] + cost[from][to];
            let next_state = state | (1 << to);
            if dp[to][next_state] > next_cost {
                dp[to][next_state] = next_cost;
                q.push_back((to, next_state));
            }
        }
    }
    println!("{}", ans);
