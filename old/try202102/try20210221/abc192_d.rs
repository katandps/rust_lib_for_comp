...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    println!("{}", max(0, ok - max_c));
}

fn check(c: i64, m: i64, x: &Vec<i64>) -> bool {
    let mut cur = x[x.len() - 1];
    let mut t = 1i64;
    for i in (0..x.len() - 1).rev() {
        t = t.saturating_mul(c);
        cur = cur.saturating_add(t.saturating_mul(x[i]));
    }
    cur <= m
}
