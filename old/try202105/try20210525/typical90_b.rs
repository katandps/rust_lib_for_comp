...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let n = reader.u();
    let mut sol = Solver { ans: Vec::new(), n };
    sol.solve(0, 0, Vec::new());
    for a in sol.ans {
        println!(
            "{}",
            a.into_iter().map(|b| if b { "(" } else { ")" }).join("")
        );
    }
}
...
impl Solver {
    fn solve(&mut self, c: usize, t: usize, buf: Vec<bool>) {
        if c == self.n {
            if t == 0 {
                self.ans.push(buf);
            }
