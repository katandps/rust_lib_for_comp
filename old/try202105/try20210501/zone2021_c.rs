...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let n = reader.u();
    let mut abcde = Vec::new();
    for _ in 0..n {
        let a = (reader.i(), reader.i(), reader.i(), reader.i(), reader.i());
        abcde.push(a);
    }
    ...
    while (ok - ng).abs() > 1 {
        ...
        for i in 0..32 {
            for j in 0..32 {
                for k in 0..32 {
                    if check[i] && check[j] && check[k] && (i | j | k) == 31 {
                        ok_bool = true;
                    }
                }
            }
