...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut ok = 1_000_000_001i64;
    let mut ng = -1;
    'case: while (ok - ng).abs() > 1 {
        let mid = (ok + ng) / 2;
        let m = a
            .iter()
            .map(|av| av.iter().map(|&ai| ai > mid).collect_vec())
            .collect_vec();
        ...
        for i in 0..n {
            for j in 0..n {
                sum[i + 1][j + 1] =
                    sum[i + 1][j] + sum[i][j + 1] - sum[i][j] + if m[i][j] { 1 } else { 0 };
            }
        }
        for i in 0..n - k + 1 {
            for j in 0..n - k + 1 {
                if sum[i + k][j + k] + sum[i][j] - sum[i + k][j] - sum[i][j + k] < k * k / 2 + 1 {
                    // dbg!(&sum, mid);
                    ok = mid;
                    continue 'case;
                }
