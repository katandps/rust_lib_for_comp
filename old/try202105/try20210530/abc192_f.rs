...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    // m種類選ぶ
    for m in 1..=n {
        // dp[n][k][a] = max of n番目までみてk個選んだ時、残りの残魔力の最小値
        let mut dp = vec![vec![vec![INF; n]; n + 1]; n + 1];
        dp[0][0][x as usize % m] = x;
        for i in 0..n {
            for j in 0..=m {
                for from in 0..m {
                    if dp[i][j][from] == INF {
                    ...
                    dp[i + 1][j][from] = min(dp[i + 1][j][from], dp[i][j][from]);
                    let to = (from as i64 - a[i]).rem_euclid(m as i64) as usize;
                    dp[i + 1][j + 1][to] = min(dp[i + 1][j + 1][to], dp[i][j][from] - a[i]);
                }
            }
        }
        if dp[n][m][0] != INF {
            ans = min(ans, dp[n][m][0] / m as i64);
        }
    }
    println!("{}", ans);
}
