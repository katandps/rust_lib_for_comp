...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for i in 0..m {
        g[a[i]].push((b[i], t[i], k[i]));
        g[b[i]].push((a[i], t[i], k[i]));
    }

    let mut fastest = vec![std::i64::MAX / 10; n];
    fastest[x - 1] = 0;
    let mut q = BinaryHeap::new();
    q.push((Reverse(0), x - 1));
    while !q.is_empty() {
        let (Reverse(t), from) = q.pop().unwrap();
        if fastest[from] < t {
            continue;
        }
        for &(to, t, k) in &g[from] {
            let time = (fastest[from] + k - 1) / k * k + t;
            if time < fastest[to] {
                fastest[to] = time;
                q.push((Reverse(time), to));
            }
        }
    }

    if fastest[y - 1] == std::i64::MAX / 10 {
        println!("{}", -1);
    } else {
        println!("{}", fastest[y - 1]);
    }
