...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let min_y = if n < m { 0 } else { n - m };
    let max_y = if n < m { n * 2 } else { n + m };

    let width = min(n, m);

    // min_yを0とする
    let mut can = vec![false; max_y - min_y + 1];
    can[width] = true;
    ...
    for (x, y) in xy {
        ...
        map.entry(x).or_insert(BTreeSet::new()).insert(y - min_y);
        pq.insert(x);
    }
    ...
    for x in pq {
        ...
        for &y in map.get(&x).unwrap() {
            if can[y] {
                if (y > 0 && can[y - 1]) || (y < can.len() - 1 && can[y + 1]) {
                    ()
                } else {
                    events.push((y, false));
                }
