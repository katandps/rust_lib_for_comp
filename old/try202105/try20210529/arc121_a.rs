...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let x = (0..n).map(|i| (xy[i].0, i)).sorted().collect_vec();
    let y = (0..n).map(|i| (xy[i].1, i)).sorted().collect_vec();

    let mut v = vec![
        (x[n - 1].0 - x[0].0, (x[n - 1].1, x[0].1)),
    ...
    v.sort();
    v.reverse();
    if (min((v[0].1).0, (v[0].1).1), max((v[0].1).0, (v[0].1).1))
        == (min((v[1].1).0, (v[1].1).1), max((v[1].1).0, (v[1].1).1))
    {
        println!("{}", v[2].0);
    } else {
        println!("{}", v[1].0);
