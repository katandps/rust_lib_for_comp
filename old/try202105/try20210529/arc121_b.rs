...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    if r.len() % 2 == 0 && g.len() % 2 == 0 && b.len() % 2 == 0 {
        println!("{}", 0);
        return;
    }
    ...
    if r.len() % 2 == 0 {
        println!(
            "{}",
            min(
                min_diff(b.clone(), g.clone()),
                min_diff(r.clone(), g.clone()) + min_diff(r.clone(), b.clone()),
            )
        );
    } else if g.len() % 2 == 0 {
        println!(
            "{}",
            min(
                min_diff(r.clone(), b.clone()),
                min_diff(r.clone(), g.clone()) + min_diff(g.clone(), b.clone()),
            )
        );
    } else {
        // b.len() % 2 == 0
        println!(
            "{}",
            min(
                min_diff(r.clone(), g.clone()),
                min_diff(r.clone(), b.clone()) + min_diff(g.clone(), b.clone()),
            )
        );
    }
}

fn min_diff(a: Vec<i64>, mut b: Vec<i64>) -> i64 {
    if a.len() == 0 || b.len() == 0 {
        return std::i64::MAX / 10;
    }
    b.sort();
    let mut ret = std::i64::MAX;
    for &ai in &a {
        // biがa以上となるiの最小値
        let mut ok = b.len() as i64;
        let mut ng = -1;
        while (ok - ng).abs() > 1 {
            let mid = (ok + ng) / 2;
        ...
        if let Some(k) = b.get(ok as usize) {
            ret = min(ret, (k - ai).abs());
        }
        if let Some(k) = b.get(ng as usize) {
            ret = min(ret, (k - ai).abs());
        }
    }
    ret
