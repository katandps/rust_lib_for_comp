...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for _ in 0..t {
        let n = reader.u();
        let p = reader.uv(n);
        let sorted = p.iter().sorted().cloned().collect_vec();
        let mut current = p.clone();
        let mut ans = vec![];
        let mut next = 1;
        let mut stack = false;
        while next < n {
            // println!("dbg {}", current.iter().join(" "));
            // println!("next {}", next);
            // dbg!(&ans);
            if current[next - 1] == next {
                if next == n - 1 {
                    break;
                } else {
                    swap(&mut current, next - 1);
                    ans.push(next);
                    swap(&mut current, next);
                    ans.push(next + 1);
                }
            }
            for i in 0..n {
                if current[i] == next {
                    if i % 2 == next % 2 {
                        (next..=i).rev().for_each(|i| {
                            ans.push(i);
                            swap(&mut current, i - 1)
                        });
                    } else {
                        if !stack {
                            swap(&mut current, next - 1);
            ...
            // dbg!(&ans.iter().join(" "), &current.iter().join(" "));
            stack = false;
            next += 1;
        }
        ...
        for j in 0..ans.len() {
            assert_ne!(ans[j] % 2, j % 2);
            swap(&mut check, ans[j] - 1);
        }
        assert!(ans.len() <= n * n);
...
fn swap(v: &mut Vec<usize>, i: usize) {
    let t = v[i];
    v[i] = v[i + 1];
    v[i + 1] = t;
}
