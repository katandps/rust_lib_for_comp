...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let a = a.into_iter().zip(0..).map(|(a, z)| a + z).collect_vec();
    let b = b.into_iter().zip(0..).map(|(b, z)| b + z).collect_vec();

    if a.iter().sorted().collect_vec() != b.iter().sorted().collect_vec() {
        println!("-1");
        return;
    }
    ...
    let mut b_i = Vec::new();
    for i in 0..n {
        let p = map.get_mut(&b[i]).unwrap();
        let Reverse(bb) = p.pop().unwrap();
        b_i.push(bb);
    }
