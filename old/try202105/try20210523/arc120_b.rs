...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let (h, w) = reader.u2();
    let s = reader.char_map(h);

    let mut count_by_dist = vec![0i64; h + w];
    let mut color_for_dist = vec!['.'; h + w];
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            if color_for_dist[x + y] == '.' {
                color_for_dist[x + y] = s[y][x];
            } else {
                if color_for_dist[x + y] != s[y][x] {
                    println!("{}", 0);
                    return;
                }
            }
    ...
    let mut ans = Mi::new(1);
    for i in 0..h + w {
        if color_for_dist[i] == '.' && count_by_dist[i] > 0 {
            ans *= 2;
        }
    }
    println!("{}", ans);
