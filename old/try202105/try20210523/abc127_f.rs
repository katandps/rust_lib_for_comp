...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    left.push(std::i64::MIN);
    let mut right = BinaryHeap::new();
    right.push(Reverse(std::i64::MAX));
    let mut b = 0;
    let mut m = 0i64;
    for _ in 0..q {
        if reader.u() == 1 {
            let (a_i, b_i) = reader.i2();
            b += b_i;
            ...
            m += if l <= a_i && a_i <= r {
                ...
                min(i64::abs(a_i - l), (a_i - r).abs())
            };

            let mut v = vec![a_i, a_i, l, r];
            v.sort();
            left.push(v[0]);
            left.push(v[1]);
            right.push(Reverse(v[2]));
            right.push(Reverse(v[3]));
        } else {
            println!("{} {}", left.iter().next().unwrap(), m + b);
        }
    }
}
