...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for &c in &column {
        *column_count.entry(c).or_insert(0) += 1;
    }
    let mut eq = 0i64;
    for (r, rc) in row_count {
        if k < r {
            continue;
        }
        if let Some(&cc) = column_count.get(&(k - r)) {
            eq += rc * cc;
        }
    }
    ...
    let mut ans = eq;
    for (r, c) in rc {
        if row[r - 1] + column[c - 1] == k {
            ans -= 1;
        }
        if row[r - 1] + column[c - 1] == k + 1 {
            ans += 1;
        }
    }
    println!("{}", ans);
