...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut buf = vec![Vec::new(); 200];

    let mut k = 200;
    let mut ans = Vec::new();
    'search: for (&ai, j) in a.iter().zip(0..) {
        let mut new_buf = vec![Vec::new(); 200];
        for i in 0..200 {
            if i == 0 {
                if !buf[(i + ai) % 200].is_empty() {
                    k = (i + ai) % 200;
                    ans = buf[i].clone();
                    ans.push(j + 1);
                    break 'search;
                } else {
                    let mut b = buf[i].clone();
                    b.push(j + 1);
                    new_buf[(i + ai) % 200] = b;
                }
            } else {
        ...
        for i in 0..200 {
            if !new_buf[i].is_empty() {
                buf[i] = new_buf[i].clone();
            }
        }
    }
    ...
    println!("Yes");
    println!("{} {}", buf[k].len(), buf[k].iter().join(" "));
    println!("{} {}", ans.len(), ans.iter().join(" "));
    //
    // println!(
    //     "{}",
    //     buf[k].iter().map(|&i| a[i as usize - 1]).sum::<usize>()
    // );
    // println!("{}", ans.iter().map(|&i| a[i as usize - 1]).sum::<usize>());
}
