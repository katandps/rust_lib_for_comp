...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let k = d.iter().max().unwrap() + 1;
    let mut bit_d = BinaryIndexedTree::new(k);
    let mut bit_t = BinaryIndexedTree::new(k);
    let mut bit_q = BinaryIndexedTree::new(k);

    for &di in &d {
        bit_d.add(di * 2, Mi::new(1));
    }
    for &di in &d {
        bit_t.add(di * 2, bit_d.sum(di));
    }
    for &di in &d {
        bit_q.add(di * 2, bit_t.sum(di));
    }
