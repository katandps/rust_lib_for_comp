...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for i in 0..n - 1 {
        for (x, count) in dp[i].clone() {
            if x % a[i + 1] == 0 {
                *dp[i + 1].entry(x).or_insert(0) += count;
            } else {
                *dp[i + 1].entry(x - x % a[i + 1]).or_insert(0) += count;
                *dp[i + 1].entry(x - x % a[i + 1] + a[i + 1]).or_insert(0) += count;
            }
        }
    }

    let mut ans = 0;
    for (_x, &count) in &dp[n - 1] {
        ans += count;
    }
    println!("{}", ans);
}
