...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    // あと何点得られるか
    let mut rest_score = vec![vec![0i64; w]; h];

    // 先に動かすのは高橋 = idousaki no x + y ga odd naraba plus no tokuten
    for i in (0..h - 1).rev() {
        rest_score[i][w - 1] = rest_score[i + 1][w - 1]
            + if a[i + 1][w - 1] ^ !((i + w) % 2 == 1) {
                1
            } else {
                -1
            };
    ...
    for i in (0..w - 1).rev() {
        rest_score[h - 1][i] = rest_score[h - 1][i + 1]
            + if a[h - 1][i + 1] ^ !((h + i) % 2 == 1) {
                1
            } else {
                -1
            };
    ...
    //
    // for i in 0..h {
    //     println!("{}", rest_score[i].iter().join(" "))
    // }
    ...
    for y in (0..h - 1).rev() {
        for x in (0..w - 1).rev() {
            if (x + y) % 2 == 0 {
                // takahashi
                rest_score[y][x] = max(
                    rest_score[y + 1][x] + if a[y + 1][x] { 1 } else { -1 },
                    rest_score[y][x + 1] + if a[y][x + 1] { 1 } else { -1 },
                );
            } else {
                //aoki
                rest_score[y][x] = min(
