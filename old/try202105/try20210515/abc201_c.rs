...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut in_vec = HashSet::new();
    let mut out_vec = HashSet::new();
    let mut ques_vec = HashSet::new();
    ...
    let mut ans = 0;
    'case: for k in 0..10000 {
        let s = format!("{:0>4}", k);

        let mut p = in_vec.clone();
        for si in s.chars() {
            let u: usize = si.to_string().parse().unwrap();
            p.remove(&u);
        }

        for si in s.chars() {
            let u: usize = si.to_string().parse().unwrap();
            if out_vec.contains(&u) {
                continue 'case;
            }
        }
        if p.len() == 0 {
