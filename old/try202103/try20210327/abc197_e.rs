...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for c in 1..=n {
        ...
        let (&l, &r) = (
            index[c].iter().min().unwrap(),
            index[c].iter().max().unwrap(),
        );
        ans += r - l;
        v.push((l, r));
    }

    // start = g[2*c] goal = g[2*c+1]
    let c = v.len(); //colors
    let mut g = vec![Vec::new(); c * 2 + 2];
    g[2 * c].push((1, v[0].0.abs()));
    g[2 * c].push((0, v[0].1.abs()));
    for i in 1..c {
        g[2 * (i - 1)].push((2 * i + 1, (v[i].0 - v[i - 1].0).abs()));
        g[2 * (i - 1)].push((2 * i, (v[i].1 - v[i - 1].0).abs()));
        g[2 * (i - 1) + 1].push((2 * i + 1, (v[i].0 - v[i - 1].1).abs()));
        g[2 * (i - 1) + 1].push((2 * i, (v[i].1 - v[i - 1].1).abs()));
    }
    g[2 * (c - 1)].push((2 * c + 1, v[c - 1].0.abs()));
    g[2 * (c - 1) + 1].push((2 * c + 1, v[c - 1].1.abs()));

    let mut q = VecDeque::new();
    q.push_front(2 * c);
    let mut dist = vec![1_000_000_000_000_000_00; c * 2 + 2];
    dist[2 * c] = 0;
    while !q.is_empty() {
