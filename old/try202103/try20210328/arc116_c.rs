...
mod factorial {
    ...
    impl Factorial {
        ...
        /// 階乗 n!
        pub fn fact(&self, number: usize) -> ModInt<Num> {
            self.stack[number]
        }
...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let fact = Factorial::new(n as i64 + 100);
    let ans = (1..=m).fold(ModInt::new(0), |sum, i| {
        let mut l = 2;
        let mut t = i;
        let mut primes = HashMap::new();
        while l * l <= t {
            if t % l == 0 {
                *primes.entry(l).or_insert(0) += 1;
                t /= l;
            } else {
                l += 1;
            }
        ...
        if t != 1 {
            *primes.entry(t).or_insert(0) += 1;
        }

        let a = primes.iter().fold(ModInt::new(1), |r, (_prime, &count)| {
            r * fact.nhk(n as i64, count)
        });
