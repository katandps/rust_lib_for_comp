...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut mod7 = vec![0; n];
    let mut t = 1;
    for i in (0..n).rev() {
        mod7[i] = t % 7;
        t = t * 10 % 7;
    }
    let mut t_win = vec![false; 7];
    t_win[0] = true;
    for i in (0..n).rev() {
        let t_turn = x[i] == 'T';
        let mut new_win = t_win.clone();
        if t_turn {
            for j in 0..7 {
                let m = ((s[i] as u8 - '0' as u8) as usize) * mod7[i] % 7;
                new_win[j] |= t_win[(j + m) % 7];
            }
        } else {
            for j in 0..7 {
                let m = ((s[i] as u8 - '0' as u8) as usize) * mod7[i] % 7;
    ...
    println!("{}", if t_win[0] { "Takahashi" } else { "Aoki" });
}
