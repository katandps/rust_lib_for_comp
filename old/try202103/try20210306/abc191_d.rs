...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let (x, y, r) = (reader.f(), reader.f(), reader.f());
    let (x, y, r) = (
        (x * 10000.0 + 5_000_000_000.1) as i64,
        (y * 10000.0 + 5_000_000_000.1) as i64,
        (r * 10000.0 + 0.1) as i64,
    );
    ...
    for i in 0..=1_000_000 {
        ...
        let mut ok = x;
        let mut ng = x + 2_000_000_000;
        while (ok - ng).abs() > 1 {
            let xx = (ok + ng) / 2;
            if (y - yy) * (y - yy) + (x - xx) * (x - xx) <= r * r {
                ok = xx;
            } else {
                ng = xx;
            }
        }
        ans += (ok - (x - x % 10000)) / 10000;
        ...
        ans += ((x + 9999) / 10000 * 10000 - ok) / 10000;
    }
    println!("{}", ans);
}
