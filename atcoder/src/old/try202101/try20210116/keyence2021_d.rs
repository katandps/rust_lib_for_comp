...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut before = vec![vec![true, false]];
    let mut current = vec![vec![false; 256]; 256];
    for i in 2..=n {
        current = vec![vec![false; 256]; 256];
        for j in 0..1 << i - 1 {
            current[0][j] = true;
        }
        for j in 0..(1 << i) - 2 {
            for k in 0..(1 << i) / 2 {
                current[j + 1][k] = before[j / 2][k];
            }
        }
        for j in 0..(1 << i) - 2 {
            for k in 0..(1 << (i - 1)) {
                ...
                //     before[j / 2][k] ^ (j % 2 == 0)
                // );
                current[j + 1][k + (1 << (i - 1))] = before[j / 2][k] ^ !(j % 2 == 0);
            }
        }
    ...
    // let mut count = vec![vec![0; 1 << n]; 1 << n];
    // for i in 0..line {
    //     for j in 0..length {
    //         for k in j + 1..length {
    //             if current[i][j] && current[i][k] {
    //                 count[j][k] += 1;
    //             }
    //         }
    //     }
    ...
    for i in 0..line {
        let mut b = Vec::new();
        for j in 0..length {
            b.push(if current[i][j] { 'A' } else { 'B' });
        }
        println!("{}", b.iter().join(""));
    }
}
