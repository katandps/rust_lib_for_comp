...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for _ in 0..m {
        let p = reader.u();
        match p {
            3 => op.push((3, reader.i())),
            4 => op.push((4, reader.i())),
            x => op.push((x, 0)),
        }
    }
    ...
    let q = reader.u();
    let ab = reader.uv2(q);
    let mut abi: Vec<(usize, usize, usize)> = ab
        .iter()
        .cloned()
        .zip(0..)
        .map(|((a, b), i)| (a, b - 1, i))
        .collect_vec();
    abi.push((10000000, 0, 0));
    abi.sort();
    ...
    for i in 0..m {
        while abi[index].0 == i {
            let (x, y) = xy[abi[index].1];
            if !reverse {
                ans[abi[index].2] = (x * orig.0 + diff.0, y * orig.1 + diff.1);
            } else {
                ans[abi[index].2] = (y * orig.1 + diff.1, x * orig.0 + diff.0);
            }
            index += 1;
        }
        let (t, u) = op[i];
        match t {
            ...
            3 => {
                if !reverse {
                    orig = (-orig.0, orig.1);
                    diff = (-diff.0 + 2 * u, diff.1);
                } else {
                    orig = (orig.0, -orig.1);
                    diff = (diff.0, -diff.1 + 2 * u);
                }
            }
            4 => {
                if !reverse {
