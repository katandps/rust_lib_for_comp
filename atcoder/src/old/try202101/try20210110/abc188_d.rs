...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let (n, c_1) = reader.u2();
    let mut abc = reader.uv3(n);
    abc.sort();
    for i in 0..n {
        abc[i] = (abc[i].0, abc[i].1 + 1, abc[i].2);
    }

    let mut current_cost = 0;
    // キーはbが小さい順に入る 値は合計コスト
    ...
    for i in 0..n {
        let next_a = abc[i];
        while !b.is_empty() {
            let next_b = b.iter().next().unwrap();
            if next_a.0 >= *next_b.0 {
                ans += (next_b.0 - current_time) * min(current_cost, c_1);
                current_cost -= next_b.1;
                let b_key = *next_b.0;
                current_time = b_key;
                b.remove(&b_key);
            } else {
        ...
        ans += (abc[i].0 - current_time) * min(current_cost, c_1);
        current_time = abc[i].0;
        current_cost += abc[i].2;
        *b.entry(abc[i].1).or_insert(0) += abc[i].2;
    }
    while !b.is_empty() {
        let next_b = b.iter().next().unwrap();
        ans += (next_b.0 - current_time) * min(current_cost, c_1);
