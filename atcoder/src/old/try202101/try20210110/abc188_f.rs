...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    println!("{}", s(y, x, &mut map));
}

fn s(y: i64, x: i64, map: &mut HashMap<i64, i64>) -> i64 {
    if map.contains_key(&y) {
        return *map.get(&y).unwrap();
    }
    if y < x {
        return x - y;
    }
    if y % 2 == 0 {
        let ret = min(y - x, s(y / 2, x, map) + 1);
        map.insert(y, ret);
        ret
    } else {
        let ret = min(
            y - x,
            min(s((y + 1) / 2, x, map) + 2, s((y - 1) / 2, x, map) + 2),
        );
        map.insert(y, ret);
        ret
    }
