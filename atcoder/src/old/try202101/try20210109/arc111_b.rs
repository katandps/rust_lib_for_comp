...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for (a, b) in ab {
        *map.entry(a).or_insert(HashMap::new()).entry(b).or_insert(0) += 1;
        *map.entry(b).or_insert(HashMap::new()).entry(a).or_insert(0) += 1;
        rest.insert(a);
        rest.insert(b);
    }
    ...
    while !rest.is_empty() {
        ...
        arrived.insert(start);
        let mut is_tree = true;
        let mut q = VecDeque::new();
        q.push_front((start, 0));
        let mut memo = HashSet::new();
        while !q.is_empty() {
            let (from, grand_from) = q.pop_front().unwrap();
            for (&to, &count) in &map[&from] {
                if count > 1 {
                    is_tree = false;
                }
                if to == grand_from {
