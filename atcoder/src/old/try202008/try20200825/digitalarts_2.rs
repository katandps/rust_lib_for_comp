...
pub fn solve<R: BufRead>(mut reader: StdinReader<R>) {
    ...
    while rest > 0 {
        if rest >= 26 {
            ...
            if !diff && index < s.len() && s[index] == ('a' as u8 + rest as u8 - 1) as char {
                ans.push(('a' as u8 + rest as u8 - 2) as char);
                diff = true;
                rest = 1;
            } else {
                ans.push(('a' as u8 + rest as u8 - 1) as char);
                rest = 0;
            }
        }
        index += 1;
