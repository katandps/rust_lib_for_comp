...
pub fn solve<R: BufRead>(mut reader: StdinReader<R>) {
    ...
    // ans[x][i] i番目まで見たときの非負整数(a,b)の組の数
    // x=0のとき、iより前の桁でlより小さい数字が入っている(=0、1のどちらでもよい)
    let mut ans = vec![vec![0; l.len() + 1]; 2];
    ans[0][0] = 1;
    for i in 0..l.len() {
        match l[i] {
            '1' => {
                ans[0][i + 1] = ans[0][i] * 2 % m;
                ans[1][i + 1] = (ans[1][i] * 3 % m + ans[0][i]) % m;
            }
            '0' => {
                ans[0][i + 1] = ans[0][i];
                ans[1][i + 1] = ans[1][i] * 3 % m;
            }
            _ => unreachable!(),
        }
    }
    println!("{}", (ans[0][l.len()] + ans[1][l.len()]) % m);
}
