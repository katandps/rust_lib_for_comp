...
fn main() {
    ...
    let mut leftv = vec![0; tsum + 1];
    let mut rightv = vec![0; tsum + 1];
    ...
    let mut total = vec![0; tsum + 1];
    for i in 0..=tsum {
        total[i] = min(leftv[i], rightv[i]);
    }

    let mut a = total.clone();
    for i in 0..tsum {
