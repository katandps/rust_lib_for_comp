...
fn main() {
    ...
    //桁DP
    // dp[i][j][k] = 個数
    // iはnより小さいものかどうか、 jは今までに出た0以外の個数、 kは桁数
    let mut dp = vec![vec![vec![0usize; n.len() + 1]; 4]; 2];
    dp[0][0][0] = 1;
    for i in 0..n.len() {
        let a = (n[i] as u8 - '0' as u8) as usize;
        dp[0][3][i + 1] = dp[0][3][i];
        dp[1][3][i + 1] = dp[1][3][i];
        for k in (0..3).rev() {
            // n以下になるかどうかが決まっていないもの
            if a >= 1 {
                // 0
                dp[1][k][i + 1] += dp[0][k][i];
                // 1 .. a-1
                dp[1][k + 1][i + 1] += dp[0][k][i] * (a - 1);
                // a
    ...
    println!("{}", dp[0][k][n.len()] + dp[1][k][n.len()]);
}
