...
fn main() {
    ...
    // p[小さい方の数字] = BTreeMap<大、個数>
    let mut p = BTreeMap::new();
    let mut ans = a[0];
    let mut m1 = BTreeMap::new();
    m1.insert(Reverse(a[0]), 2);
    p.insert(Reverse(a[1]), m1);
    for i in 2..n {
        let cur = Reverse(a[i]);
        let (&small, m) = p.iter().next().unwrap();
        let mut m = p.entry(small).or_insert(BTreeMap::new());
        let (&big, &count) = m.iter().next().unwrap();
        if count == 1 {
            m.remove(&big);
        } else {
            *m.entry(big).or_insert(1) -= 1;
        }
        if m.is_empty() {
            p.remove(&small);
        }
        if !p.contains_key(&cur) {
            p.insert(cur, BTreeMap::new());
        }
        *p.entry(cur)
