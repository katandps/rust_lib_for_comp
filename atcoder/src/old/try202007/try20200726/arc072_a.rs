...
fn main() {
    ...
    //plus-minus-plus-minusパターンと、minus-plus-minus-plusパターンを考える
    let mut plus_sum = 0;
    let mut minus_sum = 0;
    let mut plus_ans = 0;
    let mut minus_ans = 0;
    for i in 1..=n {
        if i % 2 == 0 {
            if s[i] + plus_sum >= 0 {
                plus_ans += s[i] + plus_sum + 1;
                plus_sum -= s[i] + plus_sum + 1;
            }
            if s[i] + minus_sum <= 0 {
                minus_ans += 1 - (s[i] + minus_sum);
                minus_sum += 1 - (s[i] + minus_sum);
            }
        } else {
            if s[i] + plus_sum <= 0 {
                plus_ans += 1 - (s[i] + plus_sum);
                plus_sum += 1 - (s[i] + plus_sum);
            }
            if s[i] + minus_sum >= 0 {
                minus_ans += s[i] + minus_sum + 1;
                minus_sum -= s[i] + minus_sum + 1;
            }
        }
    }
    println!("{}", min(plus_ans, minus_ans));
}
