...
fn main() {
    ...
    let s = reader.s();
    let N = s.iter().filter(|&&c| c == 'N').count() > 0;
    let S = s.iter().filter(|&&c| c == 'S').count() > 0;
    let W = s.iter().filter(|&&c| c == 'W').count() > 0;
    let E = s.iter().filter(|&&c| c == 'E').count() > 0;
    println!("{}", if (N ^ S) || (W ^ E) { "No" } else { "Yes" });
}
