...
fn main() {
    ...
    let (h, w, a, b) = reader.u4();
    let mut memo = vec![ModInt::new(0); h - a];
    let fact = Factorial::new(h + w + 2);
    for i in 0..h - a {
        memo[i] = fact.fact(b + i - 1) / fact.fact(b - 1) / fact.fact(i);
    }
    let mut ans = ModInt::new(0);
    for i in 0..h - a {
        ans = ans
            + fact.fact(w - b - 1 + h - i - 1) / fact.fact(w - b - 1) / fact.fact(h - i - 1)
                * memo[i];
    }
    println!("{}", ans);
}
