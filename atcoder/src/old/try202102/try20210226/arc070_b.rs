...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let (n, k) = (reader.u(), reader.i() as i32);
    let mut a = reader
        .iv(n)
        .iter()
        .map(|ai| min(k, *ai as i32))
        .collect_vec();
    ...
    const LEFT: usize = 0;
    const RIGHT: usize = 1;
    let mut dp = vec![vec![vec![false; k as usize + 1]; n + 2]; 2];

    dp[LEFT][0][0] = true;
    dp[RIGHT][n + 1][0] = true;
    for i in 1..=n {
        let mut v = dp[LEFT][i - 1].clone();
        for j in 0..k {
            if dp[LEFT][i - 1][j as usize] {
                v[min(k, j + a[i - 1]) as usize] = true;
            }
        }
        dp[LEFT][i] = v;
        let mut v = dp[RIGHT][n - i + 2].clone();
        for j in 0..k {
            if dp[RIGHT][n - i + 2][j as usize] {
                v[min(k, j + a[n - i]) as usize] = true;
            }
        }
        dp[RIGHT][n - i + 1] = v;
    }
    ...
    for i in 1..=n {
        let ai = a[i - 1];
        let right = (0..k)
            .filter(|&j| dp[RIGHT][i + 1][j as usize])
            .collect::<BTreeSet<_>>();
        for left in 0..k {
            if !dp[LEFT][i - 1][left as usize] {
                continue;
            }
            let ma = max(0, k - left);
            let mi = max(0, k - left - ai);
            if right.range(mi..ma).count() > 0 {
                ans -= 1;
                break;
            }
        }
