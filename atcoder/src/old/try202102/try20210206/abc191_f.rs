...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for &ai in &a {
        let mut l = 1;
        while l * l <= ai && l < minimum {
            if ai % l == 0 {
                if ai / l < minimum {
                    divs.entry(ai / l).or_insert(HashSet::new()).insert(l);
                }
                divs.entry(l).or_insert(HashSet::new()).insert(ai / l);
            }
            l += 1;
        }
    }

    for (div, another) in divs {
        let mut g = *another.iter().next().unwrap();
        for &ano in &another {
            g = gcd(g, ano);
        }
        if g == 1 {
