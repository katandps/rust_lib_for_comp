...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    const INF: usize = std::usize::MAX / 1000;
    // (toutyaku)
    ...
    pq.push((Reverse(0), x));
    while !pq.is_empty() {
        let (Reverse(time), from) = pq.pop().unwrap();
        if dist[from] < time {
            continue;
        }
        ...
        for &(to, t, k) in &g[from] {
            let now = dist[from];
            let start = (now + k - 1) / k * k;
            let attend = start + t;
            if dist[to] <= attend {
                continue;
            } else {
                dist[to] = attend;
                pq.push((Reverse(attend), to));
            }
        }
    }
...
pub mod reader {
    ...
    impl<R: BufRead> Reader<R> {
        ...
        vec_methods! {
            uv: usize; uv2: (usize, usize); uv3: (usize, usize, usize);uv4:(usize,usize,usize,usize);
