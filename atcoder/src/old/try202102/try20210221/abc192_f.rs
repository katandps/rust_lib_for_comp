...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for m in 1..=n as i64 {
        let mut dp = vec![vec![vec![-1; n]; n + 1]; n + 1];
        dp[0][0][0] = 0;
        for i in 1..=n {
            for j in 1..=min(i, m as usize) {
                for from in 0..m as usize {
                    dp[i][j][from] = max(dp[i][j][from], dp[i - 1][j][from]);
                    if dp[i - 1][j - 1][from] == -1 {
                        continue;
                    }
                    let i = i as usize;
                    let to = (from + a[i - 1] as usize) % m as usize;
                    dp[i][j][to] = max(dp[i][j][to], dp[i - 1][j - 1][from] + a[i - 1])
                }
            }
        }

        // dbg!(&dp);
        if dp[n][m as usize][x % m as usize] == -1 {
            continue;
        }
        ans = min(ans, (x as i64 - dp[n][m as usize][x % m as usize]) / m);
    }
    println!("{}", ans);
}
