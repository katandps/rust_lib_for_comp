...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut b = (0..n as i64).zip(a).map(|(a, b)| (b, a)).collect_vec();
    b.sort();
    b.reverse();
    ...
    for len in 1..=n {
        ...
        for i in 0..=len {
            let left = i as i64 - 1;
            let right = n as i64 - (len as i64 - i as i64);
            if i == len {
                dp[i][0] = dp[i - 1][0] + m * (left - index).abs();
                continue;
            }
            if i == 0 {
                dp[0][len] = dp[0][len - 1] + m * (right - index).abs();
                continue;
            }

            dp[i][len - i] = max(
                dp[i - 1][len - i] + m * (left - index).abs(),
                dp[i][len - i - 1] + m * (right - index).abs(),
            )
        }
    }
