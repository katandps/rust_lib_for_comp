...
fn main() {
    ...
    let mut small: Vec<_> = ab.iter().map(|(a, b)| a).cloned().collect();
    let mut big: Vec<_> = ab.iter().map(|(a, b)| b).cloned().collect();
    small.sort();
    big.sort();

    if n % 2 == 0 {
        let s = small[n / 2] + small[n / 2 - 1];
        let b = big[n / 2] + big[n / 2 - 1];
        println!("{}", (b - s) + 1);
    //dbg!(&s, &b);
    } else {
        let s = small[n / 2];
