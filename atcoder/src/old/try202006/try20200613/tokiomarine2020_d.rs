...
fn main() {
    ...
    // 根から深さ10までのすべてのナップサックをやっておく
    let mut knap = vec![Vec::new()];
    let m = min(1024, n);
    for i in 1..=m {
        knap.push(knapsack(&vw, i, 0));
    }
...
// Vec[重み] = 可能な価値の最大値 のVectorを返す全探索ナップサックを行う
fn knapsack(vw: &Vec<(usize, usize)>, mut l: usize, m: usize) -> Vec<usize> {
    let mut pattern = Vec::new();
    while l > m {
        pattern.push((vw[l - 1]));
        l /= 2;
