...
fn main() {
    ...
    // あるiより右側にある項のうち、最大のもの
    let mut m = vec![0; n + 1];
    for i in 0..n {
        m[n - i - 1] = m[n] + a[n - i - 1];
    }

    let mut mi = *a.iter().max().unwrap();
    for i in 0..n {
        let a_i = a[i];
        pos.entry(a_i).or_insert(Vec::new()).push(i);
        mi = min(mi, a_i);
        diff = max(m[i] - mi, diff);
    }
    let mut ans = 0;
    for (&k, set) in &pos {
        if !pos.contains_key(&(k + diff)) {
            continue;
        }
        let vs = &pos[&(k + diff)];
        let mut m = set.len();
        ...
        while index < set.len() {
            while vs[other_index] < set[index] {
                other_index += 1;
                if other_index == vs.len() {
                    break;
                }
            }

            m = min(m, index + vs.len() - other_index);
            index += 1;
            if other_index == vs.len() {
                //println!("{}", "br");
                break;
