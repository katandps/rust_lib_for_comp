...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for c in s {
        *cnt.entry(c as usize - '0' as usize).or_insert(0) += 1;
    }

    if d == 1 {
        if cnt.contains_key(&8) {
            println!("{}", "Yes");
        } else {
            println!("{}", "No");
        }
    } else if d == 2 {
        'two: for i in 0..100 / 8 {
            let i = i * 8;
            ...
            *using.entry(one).or_insert(0) += 1;
            *using.entry(two).or_insert(0) += 1;

            for (key, count) in using {
                if !cnt.contains_key(&key) {
                    continue 'two;
                }
                if *cnt.get(&key).unwrap() < count {
                    continue 'two;
                }
            }
            println!("{}", "Yes");
        ...
        'three: for i in 0..1000 / 8 {
            ...
            *using.entry(one).or_insert(0) += 1;
            *using.entry(two).or_insert(0) += 1;
            *using.entry(three).or_insert(0) += 1;
