...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let (n, m) = reader.uu();
    let mut h = reader.iv(n);
    let mut w = reader.iv(m);
    h.sort();
    w.sort();

    // 左から2i人、隣り合った人で組ませたときに生じる身長差
    ...
    for i in 0..n / 2 {
        ...
        right.push((h[n - 1 - 2 * i] - h[n - 2 - 2 * i]).abs() + right[i]);
    }
    ...
    for i in 0..m {
        ...
        if ng >= 0 && (ng as usize) < n {
            check.push(ng)
        }
        if ok >= 0 && (ok as usize) < n {
            check.push(ok);
        }

        for check in check {
            let mut a = (h[check as usize] - w[i]).abs();
            if check % 2 == 0 {
                a += left[check as usize / 2];
                a += right[(n - check as usize) / 2];
            } else {
                a += left[check as usize / 2];
                a += right[(n - check as usize) / 2 - 1];
                a += (h[check as usize - 1] - h[check as usize + 1]).abs();
            }
