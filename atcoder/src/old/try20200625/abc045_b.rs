...
fn main() {
    ...
    let (s1, s2, s3) = (reader.s(), reader.s(), reader.s());

    let mut s1 = s1.iter().cloned().collect::<VecDeque<char>>();
    let mut s2 = s2.iter().cloned().collect::<VecDeque<char>>();
    let mut s3 = s3.iter().cloned().collect::<VecDeque<char>>();
    ...
    loop {
        if k[&cur].is_empty() {
            println!("{}", cur.to_uppercase());
            return;
        }
        cur = k.entry(cur).or_insert(VecDeque::new()).pop_front().unwrap();
    }
}
