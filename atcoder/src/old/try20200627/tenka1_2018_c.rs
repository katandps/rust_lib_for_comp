...
fn main() {
    ...
    let n = reader.u();
    let mut a = reader.iv(n);
    a.sort();

    let mut ans1 = VecDeque::new();
    let mut ans2 = VecDeque::new();
    let mut v1 = a.iter().cloned().collect::<VecDeque<_>>();
    let mut v2 = a.iter().cloned().collect::<VecDeque<_>>();

    // 中央に最小
    ans1.push_front(v1.pop_front().unwrap());
    // 中央に最大
    ans2.push_front(v2.pop_back().unwrap());

    let mut i = 0;
    while !v1.is_empty() {
        match i % 4 {
            0 => {
                ans1.push_front(v1.pop_back().unwrap());
                ans2.push_front(v2.pop_front().unwrap());
            }
            1 => {
                ans1.push_back(v1.pop_back().unwrap());
                ans2.push_back(v2.pop_front().unwrap());
            }
            2 => {
                ans1.push_front(v1.pop_front().unwrap());
                ans2.push_front(v2.pop_back().unwrap());
            ...
            3 => {
                ans1.push_back(v1.pop_front().unwrap());
                ans2.push_back(v2.pop_back().unwrap());
            }
            _ => unreachable!(),
        }
        i += 1;
    ...
    let mut score2 = 0;
    for i in 0..n - 1 {
        score1 += (ans1[i + 1] - ans1[i]).abs();
        score2 += (ans2[i + 1] - ans2[i]).abs();
    }
    println!("{}", max(score1, score2));
}
