...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let (n, m, k, q) = reader.u4();
    let pt = reader.uv2(n);

    let mut using = Vec::new();
    let mut using_sum = vec![0];

    let mut unuse = Vec::new();
    let mut unuse_sum = vec![0];
    ...
    for i in 0..unuse.len() {
        unuse_sum.push(unuse_sum[i] + unuse[i]);
    }

    for i in 0..using.len() {
        using_sum.push(using_sum[i] + using[i] + if i % k == 0 { q } else { 0 });
    }
    ...
    for i in 0..=m {
        let l = i;
        let r = m - i;
        if l >= using_sum.len() || r >= unuse_sum.len() {
            continue;
        }
        ans = min(ans, using_sum[l] + unuse_sum[r]);
    }
    println!("{}", ans)
}
