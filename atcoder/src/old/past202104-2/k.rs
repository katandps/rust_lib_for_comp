...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    const INF_MIN: i64 = std::i64::MIN / 10000;
    let mut dp = vec![INF_MIN; 100];
    dp[0] = 0;
    for (p, u) in pu {
        let mut new_dp = vec![0; 100];
        for r in 0..100 {
            let ticket = p / 100 + (p % 100 + r) / 100;
            let new_r = ((p + r) % 100) as usize;
            let t = max(dp[new_r], dp[r as usize] + u + ticket * 20 - p);
            new_dp[new_r] = t;
        }
        dp = new_dp;
    }
