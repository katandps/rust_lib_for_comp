...
pub fn solve<R: BufRead>(mut reader: StdinReader<R>) {
    let s = reader.s();
    let mut y = (0..4usize).fold(0, |x, a| {
        x + (0..a).fold(1, |b, _| b * 10) * s[3 - a].to_digit(10).unwrap()
    });
    let mut m = (0..2usize).fold(0, |x, a| {
        x + (0..a).fold(1, |b, _| b * 10) * s[6 - a].to_digit(10).unwrap()
    });
    let mut d = (0..2usize).fold(0, |x, a| {
        x + (0..a).fold(1, |b, _| b * 10) * s[9 - a].to_digit(10).unwrap()
    });
