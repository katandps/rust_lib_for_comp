...
pub fn solve<R: BufRead>(mut reader: StdinReader<R>) {
    ...
    // dp[i][j]: xからi個差まで許容したときの誤差幅jになる組み合わせの数
    const MAX_DIFF: usize = 127500;
    let mut dp = vec![vec![0usize; MAX_DIFF + 1]; n];
    dp[0][0] = 1;
    for i in 1..n {
        for j in 0..=k {
            let minor = i;
            let major = n - i;
            for k in 0..=min(
                minor * (minor + 1) / 2 * k - minor * j,
                major * (major + 1) / 2 * k - major * j,
            ) {
                dp[i][k + i * j] += dp[i - 1][k];
                dp[i][k + i * j] %= m;
            }
    ...
    for i in 1..=n {
        ...
        let major = n - i;
        for j in 1..=MAX_DIFF {
            ans += (k + 1) * (dp[minor][j] * dp[major][j] % m) % m;
            ans %= m;
        }
        println!("{}", ans);
    }
