...
pub fn solve<R: BufRead>(mut reader: StdinReader<R>) {
    let (n, l) = reader.u2();
    let mut kuji: Vec<Vec<char>> = Vec::new();
    let mut buf = Vec::new();
    for _ in 0..l {
        reader._read_next_line();
        for &c in &reader.buf {
            buf.push(c);
        }
        let utf8 = from_utf8(buf.as_slice()).unwrap().clone();
        kuji.push(utf8.parse::<String>().unwrap().chars().collect());
        buf = Vec::new();
    }
    reader._read_next_line();
    let y = from_utf8(reader.buf.as_slice()).unwrap().to_string();
...
pub mod stdin_reader {
    ...
    impl<R: BufRead> StdinReader<R> {
        ...
        pub fn _read_next_line(&mut self) {
            self.pos = 0;
            self.buf.clear();
